...
class InvestorPositionAnalyzer:
    ...
    def _import_investor_apis(self):
        ...
        try:
            ...
            from domestic_stock.inquire_investor_daily_by_market.inquire_investor_daily_by_market import inquire_investor_daily_by_market
            from domestic_stock.foreign_institution_total.foreign_institution_total import foreign_institution_total
            from domestic_stock.inquire_investor_time_by_market.inquire_investor_time_by_market import inquire_investor_time_by_market
    ...
    def analyze_daily_position(self, stock_code: str, target_date: str = None) -> Dict[str, Any]:
        """당일 투자자별 포지션 분석"""
        try:
            ...
            self.logger.error(f"당일 포지션 분석 실패 ({stock_code}, {target_date}): {e}")
            return {}
            ...
            if daily_analysis:
                ...
                for investor_type in ['foreign', 'institution', 'individual']:
                    if investor_type in daily_analysis:
                        ...
                        cumulative_data[investor_type]['net_volume'] = data.get('net_volume', 0)
                        cumulative_data[investor_type]['net_amount'] = data.get('net_amount', 0)
                        cumulative_data[investor_type]['daily_data'] = [data]  # 현재는 당일 데이터만
    ...
    def interpret_position_context(self, daily_data: Dict[str, Any], cumulative_data: Dict[str, Any]) -> str:
        """포지션 패턴 해석 및 컨텍스트 제공"""
        try:
            ...
            if 'institution' in daily_data and 'institution' in cumulative_data:
                ...
                if daily_net > 0 and cumulative_net > 0:
                    interpretation_parts.append("🏦 기관: 당일 및 30일간 지속적 순매수 (기관 관심 지속)")
                elif daily_net < 0 and cumulative_net > 0:
                    interpretation_parts.append(f"🏦 기관: 당일 순매도 {abs(daily_net/100000000):.1f}억원이지만, 30일간 누적 순매수 {cumulative_net/100000000:.1f}억원 (일시적 매도, 전체적으로는 매수 우위)")
                elif daily_net > 0 and cumulative_net < 0:
                    interpretation_parts.append("🏦 기관: 당일 순매수로 전환, 30일간 패턴 변화 시작")
                else:
    ...
    def calculate_position_score(self, daily_data: Dict[str, Any], cumulative_data: Dict[str, Any]) -> float:
    ...
    def analyze_comprehensive_position(self, stock_code: str) -> PositionAnalysisResult:
        ...
        try:
            ...
            # 3. 해석 생성
            interpretation = self.interpret_position_context(daily_analysis, cumulative_analysis)
            ...
            self.logger.info(f"종목 {stock_code} 투자자 포지션 분석 완료 (점수: {score:.2f})")
            return result
    ...
    def get_market_wide_trends(self, date: str = None) -> Dict[str, Any]:
        ...
        try:
            ...
            market_analysis = {
                'date': date,
                'kospi_trends': kospi_trends.to_dict() if kospi_trends is not None else {},
                'kosdaq_trends': kosdaq_trends.to_dict() if kosdaq_trends is not None else {},
                'aggregate_data': aggregate_data.to_dict() if aggregate_data is not None else {},
                'summary': self._generate_market_summary(kospi_trends, kosdaq_trends, aggregate_data)
            }
