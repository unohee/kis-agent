...
class InvestorPositionDB:
    ...
    def _initialize_database(self):
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                # 인덱스 생성
                cursor.execute('CREATE INDEX IF NOT EXISTS idx_stock_date ON investor_positions(stock_code, date)')
                cursor.execute('CREATE INDEX IF NOT EXISTS idx_date ON investor_positions(date)')
                ...
                cursor.execute('CREATE INDEX IF NOT EXISTS idx_market_date ON market_trends(date, market_type)')
                
                conn.commit()
                self.logger.info(f"투자자 포지션 데이터베이스 초기화 완료: {self.db_path}")
    ...
    def save_daily_position(self, record: InvestorPositionRecord) -> bool:
        ...
        try:
            ...
            with sqlite3.connect(self.db_path) as conn:
                ...
                ''', (
                    record.stock_code, record.date,
                    record.foreign_buy_vol, record.foreign_sell_vol, record.foreign_net_vol,
                    record.foreign_buy_amt, record.foreign_sell_amt, record.foreign_net_amt,
                    record.institution_buy_vol, record.institution_sell_vol, record.institution_net_vol,
                    record.institution_buy_amt, record.institution_sell_amt, record.institution_net_amt,
                    record.individual_buy_vol, record.individual_sell_vol, record.individual_net_vol,
                    record.individual_buy_amt, record.individual_sell_amt, record.individual_net_amt,
                    record.current_price, record.trading_volume, record.updated_at
                ))
                
                conn.commit()
                self.logger.debug(f"포지션 데이터 저장 완료: {record.stock_code} ({record.date})")
                return True
                
        except sqlite3.Error as e:
            self.logger.error(f"포지션 데이터 저장 실패 ({record.stock_code}, {record.date}): {e}")
            return False

    def get_30day_positions(self, stock_code: str, end_date: str = None) -> List[InvestorPositionRecord]:
        """30일간 포지션 데이터 조회"""
        try:
            if end_date is None:
            ...
            with sqlite3.connect(self.db_path) as conn:
                ...
                self.logger.debug(f"30일 포지션 데이터 조회 완료: {stock_code} ({len(records)}건)")
                return records
    ...
    def get_position_by_date(self, stock_code: str, date: str) -> Optional[InvestorPositionRecord]:
        """특정 날짜 포지션 데이터 조회"""
        try:
            with sqlite3.connect(self.db_path) as conn:
                cursor = conn.cursor()
    ...
    def save_market_trend(self, date: str, market_type: str, trend_data: Dict[str, Any]) -> bool:
        """시장 전체 동향 저장"""
        try:
            with sqlite3.connect(self.db_path) as conn:
                cursor = conn.cursor()
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                for row in rows:
                    ...
                    # 전체 합계 계산
                    for key in ['foreign_net_amt', 'institution_net_amt', 'individual_net_amt', 'total_trading_value']:
    ...
    def cleanup_old_data(self, days_to_keep: int = 90) -> None:
        ...
        try:
            ...
            with sqlite3.connect(self.db_path) as conn:
                ...
                # 오래된 포지션 데이터 삭제
                cursor.execute('DELETE FROM investor_positions WHERE date < ?', (cutoff_date,))
                position_deleted = cursor.rowcount
                
                # 오래된 시장 동향 데이터 삭제
                cursor.execute('DELETE FROM market_trends WHERE date < ?', (cutoff_date,))
                trend_deleted = cursor.rowcount
                
                conn.commit()
                
                self.logger.info(f"오래된 데이터 정리 완료: 포지션 {position_deleted}건, 시장동향 {trend_deleted}건 삭제")
                return True
    ...
    def get_database_stats(self) -> Dict[str, Any]:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                cursor.execute('SELECT COUNT(DISTINCT stock_code) FROM investor_positions')
    ...
    def export_data(self, stock_code: str = None, start_date: str = None, end_date: str = None) -> List[Dict[str, Any]]:
        """데이터 내보내기 (JSON 형태)"""
        try:
            with sqlite3.connect(self.db_path) as conn:
                cursor = conn.cursor()
