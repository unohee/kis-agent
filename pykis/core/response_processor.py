...
class DictResponseProcessor(ResponseProcessor):
    ...
    def process(self, response: Dict, field_type: Optional[str] = None) -> Optional[Dict]:
...
class DataFrameResponseProcessor(ResponseProcessor):
    ...
    def process(self, response: Dict, field_type: Optional[str] = None) -> Optional[pd.DataFrame]:
    ...
    def _convert_to_dataframe(self, output_data: Union[List, Dict]) -> Optional[pd.DataFrame]:
...
class ResponseProcessorFactory:
    ...
    @staticmethod
    def create_processor(return_dataframe: bool, metadata_adder=None, field_converter=None) -> ResponseProcessor:
        """
        응답 타입에 따른 처리기 생성
        ...
        if return_dataframe:
            if not metadata_adder or not field_converter:
                raise ValueError("DataFrame 처리기는 metadata_adder와 field_converter가 필요합니다")
