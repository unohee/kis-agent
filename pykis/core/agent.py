...
class Agent:
    ...
    def get_index_daily_price(self, index_code: str = "0001", end_date: str = None, period: str = "D"):
        """
        국내 지수 일자별 시세 조회
        ...
        methods_info = {
            'stock': {
                ...
                'methods': [
                    ...
                    ('get_daily_minute_price', '일별분봉시세조회', 'get_daily_minute_price("005930", "20250715", "153000")'),
            ...
            'utility': {
                ...
                'methods': [
                    ...
                    ('calculate_support_resistance', '매물대 분석', 'calculate_support_resistance("005930")'),
        ...
        if date is None:
            ...
            if now < market_open_time:
                ...
                prev_business_day = self._get_last_business_day(
                    (datetime.datetime.strptime(last_business_day, '%Y%m%d') - datetime.timedelta(days=1)).strftime('%Y%m%d')
                )
    ...
    def calculate_support_resistance(self, code: str, date: str = None, price_bins: int = 50) -> dict:
        ...
        # 숫자형 변환
        numeric_columns = ['stck_oprc', 'stck_hgpr', 'stck_lwpr', 'stck_prpr', 'cntg_vol']
        ...
        result = {
            ...
            'vwap': float(vwap),
            'support_levels': [{'price': float(level), 'strength': float(strength)} 
                             for level, strength in zip(support_levels, support_strength)],
            'resistance_levels': [{'price': float(level), 'strength': float(strength)} 
        ...
        logging.info(f"[{code}] 매물대 분석 완료: 지지선 {len(support_levels)}개, 저항선 {len(resistance_levels)}개")
        return result
    ...
    def _calculate_volume_profile(self, df: 'pd.DataFrame', bins: int = 50) -> list:
        ...
        # 결과 반환
        return [{'price': float((price_bins[i] + price_bins[i + 1]) / 2), 
    ...
    def _calculate_level_strength(self, df: 'pd.DataFrame', levels: list) -> list:
        ...
        for level in levels:
            ...
            touch_count = len(df[
                ((df['stck_hgpr'] >= level - price_range) & (df['stck_hgpr'] <= level + price_range)) |
                ((df['stck_lwpr'] >= level - price_range) & (df['stck_lwpr'] <= level + price_range))
            ])
