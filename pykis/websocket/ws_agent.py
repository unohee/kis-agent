...
class WSAgent:
    ...
    def __init__(self, 
        ...
        # 구독 관리
        self.subscriptions: Dict[str, Subscription] = {}  # key: f"{sub_type.value}_{key}"
        ...
        try:
            ...
            try:
                ...
                if subscription and subscription.handler:
                    await self._call_handler(subscription.handler, parsed_data, subscription.metadata)
                    
                # 타입별 핸들러 실행
                if sub_type in self.type_handlers:
                    for handler in self.type_handlers[sub_type]:
                        await self._call_handler(handler, parsed_data, subscription.metadata if subscription else {})
                        
                # 기본 핸들러 실행
                if self.default_handler:
                    await self._call_handler(self.default_handler, parsed_data, {"tr_id": tr_id, "tr_key": tr_key})
