...
class KisWebSocket:
    ...
    def __init__(
        ...
        self.trade_log_file = os.path.join(
            self.log_dir, f"trade_history_{date_str}.jsonl"
        )
    ...
    def compute_candles(self, stock_code, interval_minutes=1):
        ...
        for time_key in sorted(candles.keys()):
            ...
            candle_list.append(
                (time_key, open_price, high_price, low_price, close_price)
            )
        return candle_list
    ...
    def print_domestic_hoga(self, data):
        ...
        for i in range(9, -1, -1):
            print(
                f"{i+1:>2} | {self.format_price(sell_prices[i]):>10} | {self.format_price(sell_qty[i]):>10}"
            )
        print("-" * 40)
        for i in range(10):
            print(
                f"{i+1:>2} | {self.format_price(buy_prices[i]):>10} | {self.format_price(buy_qty[i]):>10}"
            )
        print("      매수호가       |     잔량")
        print("=" * 40)
        print(
            f"총 매도 잔량: {self.format_price(recv[43])} | 총 매수 잔량: {self.format_price(recv[44])}"
        )
        print(
            f"예상 체결가: {self.format_price(recv[47])} | 예상 체결량: {self.format_price(recv[48])} | 대비: {recv[52]}%"
        )
        print("=" * 40)
    ...
    def display_ask_bid_info(self, stock_code, data):
        ...
        try:
            ...
            if len(recv) >= 50:
                ...
                print(
                    f"매도5: {self.format_price(sell_prices[4]):<10} | 매수1: {self.format_price(buy_prices[0]):<10}"
                )
                print(
                    f"매도1: {self.format_price(sell_prices[0]):<10} | 매수5: {self.format_price(buy_prices[4]):<10}"
                )
                print("-" * 50)
        except Exception as e:
    ...
    def display_index_info(self, index_name, index_data):
        ...
        try:
            if len(index_data) >= 10:
                ...
                # 부호 처리
                change_str = (
                    f"+{change}" if not change.startswith(("+", "-")) else change
                )
                change_rate_str = (
                    f"+{change_rate}"
                ...
                print(
                    f"[{current_time}] {index_name}: {index_value} ({change_str}, {change_rate_str}%)"
                )
        except Exception as e:
    ...
    def display_trade_summary(self):
        ...
        for code, data in summary.items():
            ...
            close_price = (
                f"{data[2]:,.2f}"
                if (data[2] is not None and isinstance(data[2], (int, float)))
                else "N/A"
            )
            ...
            avg_price = (
                f"{data[5]:,.2f}"
                if (data[5] is not None and isinstance(data[5], (int, float)))
                else "N/A"
            )
            quantity = data[6] if data[6] is not None else "N/A"
            eval_amt = (
                f"{data[7]:,.2f}"
                if (data[7] is not None and isinstance(data[7], (int, float)))
                else "N/A"
            )
            ...
            ema20 = (
                f"{data[13]:,.2f}"
                if (data[13] is not None and isinstance(data[13], (int, float)))
                else "N/A"
            )
            ema120 = (
                f"{data[14]:,.2f}"
                if (data[14] is not None and isinstance(data[14], (int, float)))
                else "N/A"
            )
            ...
            print(
                f"보유 -> 매입가: {avg_price} | 수량: {quantity} | 평가금액: {eval_amt} | 매입가 대비: {purch_change}"
            )
            print(
                f"지표 -> RSI: {rsi_val} | MACD: {macd_val} | ATR: {atr_val} | 시가 대비: {daily_change}"
            )
        ...
        for code in self.stock_codes:
            ...
            if code in self.purchase_prices and cp_val is not None:
                try:
                    ...
                    purchase_change = (
                        ((cp_val - avg_price) / avg_price * 100)
                        if avg_price != 0
                        else None
                    )
                except Exception:
    ...
    async def update_holdings_loop(self):
        ...
        while True:
            try:
                ...
                if holdings:
                    new_purchase_info = {
                        h["pdno"]: (float(h["pchs_avg_pric"]), int(h["hldg_qty"]))
                        for h in holdings
                        if int(h["hldg_qty"]) > 0
                    }
                ...
                try:
                    ...
                    if len(trade_data) >= 19:
                        ...
                        self.update_trade_history(
                            stock_code, trade_time, price, strength
                        )
                except Exception as e:
            ...
            try:
                ...
                if isinstance(df, pd.DataFrame) and not df.empty:
                    ...
                    df["time"] = pd.to_datetime(
                        df["stck_bsop_date"].astype(str)
                        + df["stck_cntg_hour"].astype(str),
                        format="%Y%m%d%H%M%S",
                        errors="coerce",
                    )
                    # 체결가는 'stck_prpr'
    ...
    async def monitor_esc(self):
        ...
        while True:
            if msvcrt.kbhit():
                ...
                if key == b"\x1b":  # ESC 키
                    logging.info(
                        "ESC 키 입력 감지. 웹소켓 구독 종료 및 프로그램 종료합니다."
                    )
                    if hasattr(self, "ws") and self.ws:
                        await self.ws.close()
                    sys.exit(0)
            ...
            while True:
                ...
                if dr:
                    ...
                    if ch == "\x1b" or ch == "":  # ESC 또는 ctrl+d 감지
                        logging.info(
                            "ESC 또는 ctrl+d 감지. 웹소켓 구독 종료 및 프로그램 종료합니다."
                        )
                        if hasattr(self, "ws") and self.ws:
                            await self.ws.close()
                        sys.exit(0)
    ...
    def should_exit(self, stock_code):
        try:
            ...
            # RSI 하락 체크
            rsi_falling = (
                rsi_prev is not None and rsi_now is not None and rsi_now < rsi_prev
            )
            ...
            if rsi_falling and strength_falling and atr_high:
                ...
                logging.info(
                    f"- 체결강도 하락: {recent_strength[0]:.1f} -> {recent_strength[-1]:.1f}"
                )
            ...
            if self.should_exit(code):
                try:
                    ...
                    res = account_api.order_stock_cash(
                        ticker=code, price="0", quantity=str(qty), order_type="01"
                    )
                    logging.info(
                        f"[INFO] 응답: {res.get('msg1', '') if res else '응답 없음'}"
                    )
                except Exception as e:
    ...
    def update_price_and_indicators(self):
        ...
        if (now - self.last_balance_check).total_seconds() >= 60:
            ...
            if balance and balance.get("output1"):
                ...
                # 예수금도 함께 업데이트
                if "output2" in balance and balance.get("output2"):
                    self.initial_cash_balance = int(
                        balance["output2"][0]["dnca_tot_amt"]
                    )
        ...
        print("\n========== [보유 종목 잔고] ==========")
        print(
            f"{'종목명':<12} {'코드':<8} {'수량':>8} {'매입가':>12} {'현재가':>12} {'평가금액':>14} {'손익':>12} {'수익률':>8} {'RSI':>6} {'MACD':>8}"
        )
        print("-" * 100)
        ...
        for _, item in self.balance_info.iterrows():
            ...
            print(
                f"{name:<12} {code:<8} {quantity:>8,} {avg_price:>12,.2f} {current_price:>12,.2f} {eval_amt:>14,d} {eval_pfls_amt:>12,d} {rate_of_change:>7.2f}% {rsi_str:>6} {macd_str:>8}"
            )

        print("-" * 100)
        # 전체 수익률 계산
        total_profit_rate = (
            (total_pfls_amt / total_pchs_amt * 100) if total_pchs_amt > 0 else 0
        )

        # 예수금은 저장된 값 사용
        cash_amt = (
            self.initial_cash_balance if self.initial_cash_balance is not None else 0
        )

        print(
            f"총 평가금액: {total_eval_amt:,d}원 | 총 손익금액: {total_pfls_amt:,d}원 | 전체 수익률: {total_profit_rate:.2f}%"
        )
        print(
            f"예수금 잔액: {cash_amt:,d}원 | 총 자산: {total_eval_amt + cash_amt:,d}원"
        )
        print("=" * 100 + "\n")
                ...
                async with websockets.connect(
                    ...
                    if self.enable_index:
                        ...
                        for index_code in index_codes:
                            index_name = self.get_index_name(index_code)
                            logging.info(
                                f"[INFO] {index_name}({index_code}) 지수 구독 요청 중..."
                            )
                            sys.stdout.flush()
                            senddata_index = {
                                "header": {
                                ...
                                "body": {
                                    "input": {"tr_id": "H0IF1000", "tr_key": index_code}
                                },
                            }
                    ...
                    for stock_code in self.stock_codes:
                        ...
                        if self.enable_ask_bid:
                            senddata_hoga = {
                                ...
                                "body": {
                                    "input": {"tr_id": "H0STASP0", "tr_key": stock_code}
                                },
                            }
                        ...
                        if self.enable_program_trading:
                            senddata_program = {
                                ...
                                "body": {
                                    "input": {"tr_id": "H0GSCNT0", "tr_key": stock_code}
                                },
                            }
                            await websocket.send(json.dumps(senddata_program))
                            logging.info(
                                f"[INFO] {stock_code} 프로그램매매 구독 요청 완료"
                            )
                            sys.stdout.flush()
