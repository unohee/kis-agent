...
class TestProgramTradeAPI:
    ...
    def test_get_program_trade_by_stock_with_default_date(
        ...
        # Given
        mock_datetime.now.return_value.strftime.return_value = datetime.now().strftime(
            "%Y%m%d"
        )
    ...
    def test_get_program_trade_by_stock_with_specific_date(
        self, program_api, mock_client
    ):
    ...
    def test_get_program_trade_market_daily_success(self, program_api, mock_client):
        ...
        # Given
        expected_result = {
            "output": [{"stck_bsop_date": "20241227", "tot_pgm_shnu_vol": "1000000"}],
            "rt_cd": "0",
        }
        mock_client.make_request.return_value = expected_result
    ...
    def test_get_program_trade_by_stock_client_exception(
        self, program_api, mock_client
    ):
    ...
    def test_get_program_trade_daily_summary_client_exception(
        self, program_api, mock_client
    ):
    ...
    def test_get_program_trade_market_daily_client_exception(
        self, program_api, mock_client
    ):
