...
class TestStockMonitorIntegration:
    ...
    def test_stockmonitor_core_functions_integration(self, agent, mock_client):
        ...
        # 4. get_member (회원사 정보)
        member_response = {
            "output": [{"seln_mbcr_name": "키움증권", "seln_mbcr_amt1": "1000000"}],
            "rt_cd": "0",
        }
        ...
        agent.get_condition_stocks = Mock(
            return_value=condition_api_response["output2"]
        )
    ...
    def test_stockmonitor_program_trade_analysis_scenario(self, agent, mock_client):
        ...
        stock_price_response = {
            "output": {"stck_prpr": "60800", "acml_vol": "1000000"},  # 전체 거래량
            "rt_cd": "0",
        }
    ...
    def test_stockmonitor_foreign_exhaustion_filtering_scenario(
        self, agent, mock_client
    ):
        ...
        condition_api_response = {
            "output2": [
                ...
                {"stck_shrn_iscd": "035720", "hts_kor_isnm": "카카오"},
            ],
            "rt_cd": "0",
        }
        ...
        samsung_price_response = {
            "output": {"hts_frgn_ehrt": "25.5"},  # 25.5% (임계값 20% 이상)
            "rt_cd": "0",
        }

        kakao_price_response = {
            "output": {"hts_frgn_ehrt": "15.2"},  # 15.2% (임계값 20% 미만)
            "rt_cd": "0",
        }
