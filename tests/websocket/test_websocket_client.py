...
def test_websocket_message_handling(agent):
    ...
    """
    ws_client = agent.websocket(
        stock_codes=["005930"], enable_index=True, enable_program_trading=True
    )
        ...
        async with websockets.connect(
            ...
            if ws_client.enable_index:
                ...
                for index_code in index_codes:
                    ...
                    print(
                        f" {ws_client.get_index_name(index_code)} 지수 구독 요청 완료"
                    )
            ...
            try:
                for _ in range(30):  # 30초 대기
                    ...
                    if (
                        data
                        and "PINGPONG" not in data
                        and "SUBSCRIBE SUCCESS" not in data
                    ):
