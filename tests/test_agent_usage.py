...
def test_agent_usage():
    ...
    try:
        ...
        if not all([app_key, app_secret, account_no]):
            pytest.skip(
                "필수 API 키가 설정되지 않았습니다. 환경변수를 설정하세요: KIS_APP_KEY, KIS_APP_SECRET, KIS_ACCOUNT_NO"
            )
        ...
        if (
            samsung_investor is not None
            and hasattr(samsung_investor, "empty")
            and not samsung_investor.empty
            and hasattr(samsung_investor, "columns")
            and "raw_text" in samsung_investor.columns
        ):
            logger.info(
                f"[RAW] 삼성전자 투자자 동향 raw: {samsung_investor['raw_text']}"
            )
