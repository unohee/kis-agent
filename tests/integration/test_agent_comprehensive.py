...
def validate_api_response(response, expected_rt_cd='0', require_output=True):
    ...
    if isinstance(response, dict):
        ...
        if rt_cd == expected_rt_cd:
            if require_output:
                return ('output' in response or 'output1' in response or 'output2' in response), f"rt_cd={rt_cd}"
...
class TestUtilities:
    ...
    def test_classify_broker(self):
        ...
        for broker_name, expected_category in test_cases:
            ...
            assert expected_category in result, f"거래원 분류 오류: {broker_name} -> {result}"
