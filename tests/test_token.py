...
def test_token_functions():
    ...
    try:
        ...
        if current_token:
            print("   토큰 읽기 성공: 토큰 있음 (실제 내용은 보안상 생략)")
        else:
            print("   토큰이 없거나 만료됨")
        ...
        print("\n3. 새로운 토큰 쓰기 테스트:")
        test_token = (
            "test_token_12345_for_testing_only"  # [변경] 테스트임을 명확히 표시
        )
        test_expired = (datetime.now() + timedelta(hours=1)).strftime(
            "%Y-%m-%d %H:%M:%S"
        )
        ...
        try:
            with open(token_tmp, encoding="utf-8") as f:
                ...
                print(
                    f"   파일 구조 확인: {list(file_content.keys())}"
                )  # [변경] 실제 토큰값 대신 구조만 확인
        except Exception as e:
            print(f"   파일 읽기 오류: {e}")
