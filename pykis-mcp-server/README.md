...
```
User: 삼성전자 현재가 알려줘
Claude: (calls get_stock_price with code="005930")
...
```
User: 내 계좌 잔고 보여줘
Claude: (calls get_account_balance)
       총 자산: 10,000,000원
       예수금: 5,000,000원
       보유 종목: ...
```
...
Claude: (calls get_daily_price with code="005930")
...
Claude: (calls get_stock_investor with code="005930")
       외국인은 최근 순매수 중입니다...
```
...
Claude: (calls get_rate_limiter_status)
       초당 요청: 5/18
       분당 요청: 120/900
       ...
```
...
**해결**:
- Claude Desktop 설정 파일의 `env` 필드 확인
...
### 서버 연결 실패
**증상**: MCP 서버가 Claude Desktop에 연결되지 않음
