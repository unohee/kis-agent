...
# 전역 테스트 결과 저장소
test_results = {"success": [], "failed": [], "partial": [], "total_tests": 0}
...
def _handle_dict_result(method_name: str, result: dict, elapsed_time: float) -> dict:
    ...
    if rt_cd == "JSON_DECODE_ERROR":
        ...
        print(
            f" 부분 성공: {method_name} - rt_cd: {rt_cd}, msg: {result.get('msg1', '')}"
        )
        test_results["partial"].append(method_name)
        return result


def _handle_dataframe_result(
    method_name: str, result: pd.DataFrame, elapsed_time: float
) -> pd.DataFrame:
    """DataFrame 타입 결과 처리"""
    global test_results
    ...
    if len(result) > 0:
        ...
        if isinstance(first_item, dict):
            ...
            if len(first_item.keys()) <= 15:
                ...
                print(
                    f"   • 첫 번째 항목 키 (처음 10개): {list(first_item.keys())[:10]}..."
                )
        else:
...
# 자주 사용하는 테스트 설정들
def get_common_test_configs(
    agent, test_stock: str, test_date: str
) -> List[Dict[str, Any]]:
    """자주 사용하는 테스트 설정 반환"""
    return [
        {
            "name": "get_stock_price",
            "func": agent.get_stock_price,
            "args": [test_stock],
        },
        {
            "name": "get_daily_price",
            "func": agent.get_daily_price,
            "args": [test_stock],
        },
        {
            "name": "get_minute_price",
            "func": agent.get_minute_price,
            "args": [test_stock, "153000"],
        },
        {
            "name": "fetch_minute_data",
            "func": agent.fetch_minute_data,
            "args": [test_stock, test_date],
        },
