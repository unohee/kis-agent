...
class MinuteCandleCrawler:
    ...
    def setup_database(self):
        ...
        try:
            ...
            # 인덱스 생성
            cursor.execute(
                "CREATE INDEX IF NOT EXISTS idx_code_datetime ON minute_candles(code, datetime)"
            )
            cursor.execute(
                "CREATE INDEX IF NOT EXISTS idx_date ON minute_candles(date)"
            )
    ...
    def save_minute_data(self, df: pd.DataFrame, date: str):
        ...
        try:
            ...
            # 기존 데이터 삭제
            conn.execute(
                "DELETE FROM minute_candles WHERE code = ? AND date = ?",
    ...
    def show_statistics(self):
        ...
        try:
            ...
            # 전체 데이터 개수
            cursor.execute(
                "SELECT COUNT(*) FROM minute_candles WHERE code = ?", (self.stock_code,)
            )
        ...
        while True:
            ...
            try:
                ...
                if stock_info and "output" in stock_info:
                    self.stock_name = stock_info["output"].get(
                        "hts_kor_isnm", stock_input
                    )
