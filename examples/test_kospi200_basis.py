...
class KospiBasisTester:
    ...
    def calculate_basis(self, futures_code: str = None) -> Dict:
        ...
        futures_info = self.stock_api.get_future_option_price(
            market_div_code="F", input_iscd=futures_code
        )

        if not futures_info or futures_info.get("rt_cd") != "0":
            print(
                f" 선물 시세 조회 실패: {futures_info.get('msg1', '알 수 없는 오류') if futures_info else '응답 없음'}"
            )
            return None
    ...
    def extract_data(self, futures_info: Dict, futures_code: str) -> Optional[Dict]:
        ...
        try:
            ...
            if result["futures_price"] and result["kospi200_index"]:
                result["calculated_basis"] = (
                    result["futures_price"] - result["kospi200_index"]
                )
                result["success"] = True
    ...
    def test_multiple_codes(self):
        ...
        if results:
            ...
            for result in results:
                print(
                    f"{result['futures_code']}: 베이시스 {result['calculated_basis']:.2f}"
                )
    ...
    def run_comprehensive_test(self):
        ...
        if auto_result and auto_result["success"]:
            print(
                f" 자동 계산 성공: {auto_result['futures_code']} 베이시스 {auto_result['calculated_basis']:.2f}"
            )
        else:
            print(" 자동 계산 실패")
