...
class KospiBasisTester:
    ...
    def display_results(self, result: Dict):
        ...
        if result['calculated_basis'] is not None:
            ...
            # 베이시스 해석
            if basis > 0:
                print(f"✅ 콘탱고 상태 (정상적인 시장)")
                print(f"   해석: 선물가격이 현물가격보다 높음")
            elif basis < 0:
                print(f"❌ 백워데이션 상태 (비정상적인 시장)")
                print(f"   해석: 선물가격이 현물가격보다 낮음")
            else:
                print(f"➡️ 완전 균형 상태")
                print(f"   해석: 선물가격과 현물가격이 동일")
    ...
    def run_comprehensive_test(self):
        """종합 테스트"""
        print(f"🚀 코스피200 베이시스 계산 종합 테스트 시작")
