...
def log_results_to_json(watchlist_data, filename=None):
    ...
    try:
        ...
        logging.error(f"Failed to log results to JSON file '{filename}': {e}", exc_info=True)
# --- 결과 로깅 함수 수정 끝 ---
...
def run_monitor_once(target_date_str=None):
    ...
    try:
        ...
        if notifier: notifier.queue_message(":mag: 스캔 시작...")
        ...
        logging.warning(f"Final watchlist generated with {len(monitor.watchlist)} items.")
        print(f" -> Final watchlist generated: {len(monitor.watchlist)} items")
        sys.stdout.flush()

        processed_stocks_data = monitor.processed_stocks if hasattr(monitor, 'processed_stocks') else []

        logging.warning("Displaying processed stocks report...")
        print(" -> Calling display_detailed_watchlist_async...")
        sys.stdout.flush()
        asyncio.run(monitor.display_detailed_watchlist_async(threshold=display_threshold))
        ...
        if monitor and monitor.watchlist:
            ...
            sys.stdout.flush()
            # 로그에는 현재 시간 기록
            log_results_to_json([{**stock, 'scan_target_date': target_date_str} for stock in monitor.watchlist])
            logging.warning(f"Logged {len(monitor.watchlist)} WATCHLIST items to monitoring_results.jsonl (Target Date: {target_date_str})")
            if notifier:
        ...
        if notifier: notifier.queue_message(":white_check_mark: 스캔 성공적으로 완료.")
        ...
        if notifier: notifier.notify_error(f"모니터링 중 오류 발생: {e}")
        sys.stdout.flush()
        return None
    finally:
...
def print_strength_ranking(scan_data, scan_type="Scan"):
    ...
    try:
        ...
        for stock in scan_data:
            ...
            try:
                strength_float = float(strength_val) if strength_val is not None else -1.0
            except (ValueError, TypeError):
                strength_float = -1.0
            strength_data.append({'code': code, 'name': name, 'pch_strength': strength_float})

        strength_data_sorted = sorted(strength_data, key=lambda x: x['pch_strength'], reverse=True)

        if not strength_data_sorted:
             print("No valid strength data found.")
        else:
             max_code_len_s = max(len(s.get('code', '')) for s in strength_data_sorted) if strength_data_sorted else 6
             max_name_len_s = max(len(s.get('name', '')) for s in strength_data_sorted) if strength_data_sorted else 10
             ...
             for i, stock_info in enumerate(strength_data_sorted):
                 ...
                 if isinstance(strength_val, (int, float)) and strength_val != -1.0:
                     ...
                     strength_str = "N/A"
                 print(f"{i+1:<5} {stock_info['code']:<{max_code_len_s}} {stock_info['name']:<{max_name_len_s}} {strength_str}")
             print("-" * len(header))
        logging.info(f"Displayed {scan_type} strength ranking.")
    except Exception as analysis_err:
         logging.error(f"Error analyzing {scan_type} data: {analysis_err}", exc_info=True)
...
def get_highest_similarity_news(stock):
    ...
    highest_similarity_news = max(news_summary, key=lambda x: x.get('relevance_score', 0), default=None)
    return highest_similarity_news

def main():
    # CLI 옵션 처리
    parser = argparse.ArgumentParser(description='Stock Monitor Runner')
    parser.add_argument('--trade-monitor', action='store_true', help='Top 코드 실시간 WebSocket 모니터링 시작')
        ...
        if is_holiday_today is True:
            ...
            while attempts < max_attempts:
                 ...
                 try:
                      ...
                      logging.error(f"Error checking holiday for {check_date_str}: {holiday_check_err}. Stopping search.")
                      break
            ...
            if last_trading_day:
                ...
                try:
                    # 휴장일 스캔 실행
                    processed_data_for_ranking = run_monitor_once(target_date_str=scan_target_date)
                ...
                print("Could not determine last trading day. Cannot perform holiday scan.")
                logging.warning("Failed to determine last trading day. Exiting.")
                if notifier: notifier.queue_message(":x: 마지막 거래일을 찾지 못해 휴일 스캔 불가.")
                sys.exit(0)
            ...
            try:
                ...
                logging.error(f"Error during initial requested scan: {init_scan_err}", exc_info=True)
                print(f"\nError during initial requested scan: {init_scan_err}")
                processed_data_for_ranking = None

            # 초기 스캔 완료 후 watchlist 코드 추출
            if processed_data_for_ranking:
                initial_watchlist_codes = [s.get('code') for s in processed_data_for_ranking[:10]]  # 상위 10개
            # 초기 스캔 결과 분석 및 출력
    ...
    if offline_mode:
        ...
        if not is_trading_hours and now_time >= market_close:
            ...
            logging.warning("Market closed. Transitioning to post-market monitoring mode.")
            ...
            if now_start < target_first_scan:
                print(f"Pre-market scans will start at {target_first_scan.strftime('%H:%M')}.")
                sys.stdout.flush()
                while True:
            ...
            print("Starting pre-market scheduled scans every 90 seconds until market open.")
            sys.stdout.flush()
            ...
            while True:
                ...
                if now >= next_scan_time:
                    ...
                    try:
                        ...
                        print_strength_ranking(scheduled_data, scan_type="Pre-Market Scheduled Scan")
                    except Exception as sched_err:
                        logging.error(f"Error during pre-market scheduled scan: {sched_err}", exc_info=True)
...
def start_online_monitoring(agent, initial_watchlist_codes, args, project_root):
    ...
    if args.trade_monitor and initial_watchlist_codes:
        ...
        subprocess.Popen([
            sys.executable, os.path.join(project_root, 'KIS_WS.py'),
            '--codes', *initial_watchlist_codes,
            '--hts-id', agent.user
        ])
    print("Stock monitoring started. Runs 90 seconds after the previous scan finishes (between 09:00 and 15:30).")
        ...
        while True:
            ...
            if current_date != last_scan_date:
                ...
                if not agent.is_holiday(today_str):
                    print(f"New trading day detected: {today_str}. Scheduling 08:55 pre-market scan.")
            ...
            seconds_left = int(time_left.total_seconds()) if time_left.total_seconds() > 0 else 0
            mm, ss = divmod(seconds_left, 60)
            countdown_msg = f"다음 스캔까지 {mm:02d}:{ss:02d} 남았습니다."
            sys.stdout.write('\r' + countdown_msg + ' ' * max(0, last_countdown_len_online - len(countdown_msg)))
            sys.stdout.flush()
            ...
            if scan_now_online:
                if dt_time(9, 0) < now_time < dt_time(15, 30):
                    ...
                    print(f"Scan finished. Next scan scheduled around {next_scan_time_online.strftime('%H:%M:%S')}." )
                    # Reset the market closed notification flag if market is open again
                    market_closed_notified = False
                else:
                    if not market_closed_notified:
                        print("장시간 외입니다. 감시 루프는 유지되지만 스캔은 중단됩니다.")
