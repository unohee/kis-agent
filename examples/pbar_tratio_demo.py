...
def analyze_stock_volume_profile(code: str):
    ...
    try:
        ...
        if not pbar_result or pbar_result.get("rt_cd") != "0":
            print(
                f" 매물대 조회 실패: {pbar_result.get('msg1', '알 수 없는 오류') if pbar_result else '결과 없음'}"
            )
            return
        ...
        if "output1" in pbar_result:
            ...
            if not price_result or price_result.get("rt_cd") != "0":
                print(
                    f" 현재가 조회 실패: {price_result.get('msg1', '알 수 없는 오류') if price_result else '결과 없음'}"
                )
                return
        ...
        for item in volume_profile:
            ...
            processed_levels.append({"price": price, "volume": volume})
        ...
        # 가장 가까운 매물대 찾기
        closest_level = min(
            processed_levels[:10], key=lambda x: abs(current_price - x["price"])
        )
            ...
            if user_input.lower() in ["quit", "q", "exit"]:
                ...
                if len(results) > 1:
                    ...
                    for result in successful_results:
                        print(
                            f"   {result['code']}: {result['current_price']:,}원 "
