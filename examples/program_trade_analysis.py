...
class ProgramTradeAnalyzer:
    ...
    def analyze_program_trade(
        ...
        for _attempt in range(max_retries):
            ...
            if (
                ...
                and "output" in res
                and isinstance(res["output"], list)
            ):
                break
        ...
        if (
            not res
            or "output" not in res
            or not isinstance(res["output"], list)
            or len(res["output"]) == 0
        ):
            return None
    ...
    def _calculate_program_trade_metrics(
        self, rows: list, ref_date: str
    ) -> Optional[Dict[str, Any]]:
        """
        ...
        return {
            ...
            "today_amt_ratio": (
                round(today_net_amt / net29_amt * 100, 2) if net29_amt else None
            ),
            "program_day_shnu_vol": shnu_vol,
            "program_day_seln_vol": seln_vol,
            "program_day_total_volume": total_vol,
            "program_day_buy_ratio": (
                round(buy_ratio, 2) if buy_ratio is not None else None
            ),
        }

    def get_program_trade_trend(
    ...
    def compare_program_trade(
        self, codes: list, ref_date: Optional[str] = None
    ) -> Dict[str, Any]:
        """
...
def main():
    ...
    for code, data in comparison.items():
        if data:
            print(
                f"   {code}: 매수비율 {data['today_ratio']}%, 순매수 {data['today']:,}주"
            )
