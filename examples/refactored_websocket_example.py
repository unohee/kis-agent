...
from pykis.websocket import ClientType, EventType, WebSocketClientBuilder, WebSocketClientFactory
...
async def on_index_update(event):
    """지수 데이터 콜백"""
    data = event.data
    print(f" {data.get('name')}: {data.get('value'):.2f} ({data.get('change_rate'):+.2f}%)")
