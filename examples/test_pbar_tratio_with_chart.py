...
class PbarTratioVisualizer:
    ...
    def analyze_volume_profile(self, result: Dict, code: str) -> Dict:
        ...
        try:
            ...
            # 상위 5개 매물대 추출
            top_5_levels = sorted(
                processed_data, key=lambda x: x["volume"], reverse=True
            )[:5]
            ...
            print(
                f" 최대 거래량 매물대: {max_volume_level['price']:,}원 (거래량: {max_volume_level['volume']:.2f})"
            )
            print(" 상위 5개 매물대:")
    ...
    def compare_with_current_price(
        self, code: str, analysis: Dict, pbar_result: Dict = None
    ) -> Dict:
        """현재가와 매물대 비교 분석"""
        print(f"\n 현재가 vs 매물대 비교: {code}")
        ...
        try:
            ...
            gap_percentage = (
                (gap / max_volume_price) * 100 if max_volume_price > 0 else 0
            )
            ...
            # 가장 가까운 매물대 찾기
            closest_level = min(
                analysis["top_5_levels"], key=lambda x: abs(current_price - x["price"])
            )

            print(
                f" 가장 가까운 매물대: {closest_level['price']:,}원 (거래량: {closest_level['volume']:.2f})"
            )
    ...
    def create_volume_profile_chart(
        self, code: str, analysis: Dict, comparison: Dict
    ) -> str:
        """매물대 시각화 차트 생성"""
        ...
        try:
            ...
            for _i, level in enumerate(analysis["top_5_levels"][:3]):  # 상위 3개만
                ax1.axhline(
                    level["price"], color="green", linestyle=":", alpha=0.7, linewidth=1
                )
            ...
            if comparison["signal"] == "SUPPORT":
                ...
                ax2.text(
                    max(volumes) * 0.7,
                    max_volume_price + (max(prices) - min(prices)) * 0.05,
                    "지지 구간",
                    fontsize=12,
                    color="green",
                    fontweight="bold",
...
매물대 레벨: {analysis['total_levels']}개
    ...
    def test_single_stock_with_chart(self, code: str) -> Dict:
        ...
        chart_filename = None
        if VISUALIZATION_AVAILABLE:
            chart_filename = self.create_volume_profile_chart(
    ...
    def print_final_summary(self, results: List[Dict]):
        ...
        if failed:
            ...
            for result in failed:
                print(
                    f"   • {result['code']}: {result.get('error', '알 수 없는 오류')}"
                )
        ...
        # 차트 파일 목록
        chart_files = [
            r.get("chart_filename") for r in successful if r.get("chart_filename")
        ]
        if chart_files:
            print("\n 생성된 차트 파일:")
    ...
    def save_results(self, results: List[Dict]):
        ...
        try:
            ...
            for result in results:
                ...
                if (
                    "analysis" in save_result
                    and "all_levels" in save_result["analysis"]
                ):
                    # 큰 데이터는 제거하고 요약만 저장
