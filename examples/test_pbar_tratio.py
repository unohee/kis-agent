...
class PbarTratioTester:
    ...
    def analyze_volume_profile(self, result: Dict, code: str) -> Dict:
        ...
        try:
            ...
            # 상위 5개 매물대 추출
            top_5_levels = sorted(
                volume_profile, key=lambda x: x.get(volume_key, 0), reverse=True
            )[:5]
            ...
            print(
                f" 최대 거래량 매물대: {max_volume_price:,}원 (거래량: {max_volume:.2f})"
            )
            print(" 상위 5개 매물대:")
    ...
    def compare_with_current_price(
        ...
        try:
            ...
            print(
                f" 가장 가까운 매물대: {closest_price:,}원 (거래량: {closest_volume:.2f})"
            )
