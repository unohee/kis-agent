...
class StockMonitor:
    ...
    def calculate_volume_ratio(self, code: str) -> Optional[float]:
        ...
        try:
            ...
            # 최근 20일 평균 거래량 계산
            volumes = [
                float(item["acml_vol"]) for item in output[:20] if item.get("acml_vol")
            ]
            if not volumes:
                return None
    ...
    def check_stock(self, code: str) -> None:
        ...
        try:
            ...
            if stock_data["last_price"] is not None:
                ...
                # 거래량 급증도 계산
                if (
                    stock_data["last_volume"] is not None
                    and stock_data["last_volume"] > 0
                ):
                ...
                if price_data and "output" in price_data:
                    ...
                    try:
                        ...
                        if foreign_rate >= self.foreign_exhaustion_threshold:
                            ...
                            logging.info(
                                f" [{name}({code})] 외국인소진율 {foreign_rate:.1f}% - 통과"
                            )
                        else:
                            logging.debug(
                                f" [{name}({code})] 외국인소진율 {foreign_rate:.1f}% - 필터링"
                            )
                    except (ValueError, TypeError):
                        logging.warning(
                            f"[{name}({code})] 외국인소진율 변환 실패: {foreign_rate_str}"
                        )
