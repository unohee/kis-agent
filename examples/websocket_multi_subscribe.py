...
class MarketMonitor:
    ...
    def on_ask_bid_update(self, data: dict):
        ...
        print(f"[호가] {name}({code}): 매도1 {sell1:,.0f} | 매수1 {buy1:,.0f} | 스프레드 {spread:,.0f}")
    ...
    def on_program_trade(self, data: dict):
        ...
        if abs(net_amount) > 1_000_000_000:  # 10억 이상만 표시
            print(f"[프로그램] {name}({code}): 순매수 {net_volume:,}주 / {net_amount/100_000_000:,.1f}억")
    ...
    def display_market_status(self):
        ...
        # 종목 정보
        print("\n[감시 종목]")
        print(f"{'종목명':<12} {'현재가':>10} {'등락률':>8} {'거래량':>12} {'체결강도':>8}")
        print("-" * 60)

        for _code, data in summary['stocks'].items():
            print(f"{data['name']:<12} {data['price']:>10,.0f} {data['change_rate']:>7.2f}% "
...
async def main():
    ...
    if not all([app_key, app_secret, account_number]):
        print("환경변수를 설정해주세요: KIS_APP_KEY, KIS_APP_SECRET, KIS_ACCOUNT_NUMBER")
        return
