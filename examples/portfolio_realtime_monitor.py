...
class PortfolioRealtimeMonitor:
    ...
    def __init__(self, refresh_balance_interval: int = 300):
        ...
        self.last_balance_refresh = datetime.now() - timedelta(seconds=refresh_balance_interval)
                        ...
                        if len(time_str) >= 14:
                            ...
                            if data_time >= cutoff_time:
                                ...
                                if volume > 0:  # 거래량이 있는 데이터만
                                    self.price_volume_data[code].append((price, volume, data_time))
                    except Exception:
                        continue
    ...
    def calculate_vwap(self, code: str) -> float:
        ...
        try:
            ...
            # 윈도우 내 데이터만 필터링
            valid_data = [
                (price, volume) for price, volume, timestamp in self.price_volume_data[code]
    ...
    async def start_realtime_monitoring(self) -> None:
        ...
        try:
            ...
            async with websockets.connect(self.ws_client.url, ping_interval=30, ping_timeout=30) as websocket:
                self.ws_client.ws = websocket
                
                # 종목별 구독 요청
                for stock_code in self.positions.keys():
                    # 체결정보 구독
                    await self._subscribe_stock_trade(websocket, approval_key, stock_code)
                    
                    # 프로그램매매 구독
                    await self._subscribe_program_trade(websocket, approval_key, stock_code)
    ...
    async def _subscribe_program_trade(self, websocket, approval_key: str, stock_code: str) -> None:
        """프로그램매매 구독"""
        senddata = {
            "header": {
                "approval_key": approval_key,
    ...
    async def _process_trade_data(self, data: str) -> None:
        ...
        try:
            ...
            if stock_code in self.positions and volume > 0:
                # 가격-거래량 데이터 추가
                current_time = datetime.now()
                self.price_volume_data[stock_code].append((current_price, volume, current_time))
                
                # 오래된 데이터 정리 (윈도우 밖 데이터 제거)
                cutoff_time = current_time - timedelta(minutes=self.vwap_window_minutes)
                self.price_volume_data[stock_code] = [
                    (p, v, t) for p, v, t in self.price_volume_data[stock_code]
                    if t >= cutoff_time
                ]
                ...
                position.vwap_deviation = self.calculate_vwap_deviation(stock_code, current_price, position.vwap)
                position.market_value = position.quantity * current_price
                position.profit_loss = position.market_value - (position.quantity * position.avg_price)
    ...
    async def _refresh_balance_if_needed(self) -> None:
        """필요시 잔고 새로고침"""
        now = datetime.now()
        if (now - self.last_balance_refresh).total_seconds() >= self.refresh_balance_interval:
    ...
    def _display_portfolio_status(self) -> None:
        ...
        # 헤더
        print(f"{'종목명':<12} {'현재가':>10} {'매입가':>10} {'손익률':>8} {'VWAP':>10} {'이격률':>8} {'프로그램':>8} {'거래량':>10}")
        print("-" * 120)
        ...
        for position in self.positions.values():
            ...
            deviation_str = f"{position.vwap_deviation:+.2f}%" if position.vwap_deviation != 0 else "-"
            program_str = f"{position.program_net_ratio:+.2f}%" if hasattr(position, 'program_net_ratio') else "-"
            volume_str = f"{position.volume:,}" if position.volume > 0 else "-"
            
            print(f"{position.name:<12} "
...
async def main():
    ...
    monitor = PortfolioRealtimeMonitor(refresh_balance_interval=300)  # 5분마다 잔고 새로고침
    await monitor.run()
