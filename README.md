...
# Agent 인스턴스 생성 (실전투자, 자동 인증)
agent = Agent(
    app_key=app_key,
    app_secret=app_secret,
    account_no=account_no,
    account_code=account_code,
    # 기본값: 실전투자 URL, auto_auth=True
)
...
### 🔐 명시적 인증 (Rate Limit 방지)

여러 Agent 인스턴스를 생성하거나 토큰 발급을 제어하려면 명시적 인증을 사용하세요:
...
agent = Agent(
    ...
    account_no=account_no,
    account_code=account_code,
    auto_auth=False  # 자동 토큰 발급 비활성화
)

# 필요할 때 명시적으로 토큰 발급
token_info = agent.authenticate()
print(f"토큰 만료: {token_info['access_token_token_expired']}")

# 토큰 강제 갱신
token_info = agent.authenticate(force_refresh=True)

# 여러 Agent 인스턴스 생성 시 (Rate Limit 회피)
agent1 = Agent(..., auto_auth=True)   # 첫 번째만 자동 발급
agent2 = Agent(..., auto_auth=False)  # 저장된 토큰 재사용
agent2.authenticate()  # 기존 토큰 재사용

agent3 = Agent(..., auto_auth=False)
agent3.authenticate()  # 기존 토큰 재사용
```
