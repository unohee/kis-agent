"""
market.py - 종목 단위 시세 조회 및 주문 전용 모듈
...
✅ 의존:
- kis.core.client.KISClient: API 호출 실행기

🔗 연관 모듈:
- kis.program.trade: 프로그램 매매 정보 필터링
- kis.account.balance: 잔고 및 주문 가능 금액 확인
- kis.strategy.trigger: 실전 조건 기반 매수 트리거 실행
...
class StockAPI:
    ...
    def get_stock_investor_detail(self, ticker: str = '', retries: int = 10, force_refresh: bool = False) -> Optional[pd.DataFrame]:
        """투자자별 매매 동향 상세 조회 (항상 실시간 API 호출)
    ...
    def get_foreign_broker_net_buy(self, code: str, foreign_brokers=None, date: str = None) -> Optional[tuple]:
        """
        회원사 종목매매동향 API를 활용해 외국계 브로커의 순매수(매수-매도) 합계를 집계합니다.
        code: 종목코드
        ...
        # 회원사 종목매매동향 전체 조회
        member_daily = self.get_member_transaction(code, "99999")
        if not member_daily or "output" not in member_daily:
            return None
        df = pd.DataFrame(member_daily["output"])
        if df.empty or "memb_name" not in df.columns:
            return None
        # 외국계 브로커만 필터링 (부분 문자열 매칭 포함)
        mask = df["memb_name"].apply(lambda x: any(b in str(x) for b in foreign_brokers))
        df_foreign = df[mask].copy()
        if df_foreign.empty:
            return 0, df_foreign
        # 매수/매도/순매수 집계
        for col in ["buy_qty", "sell_qty"]:
            df_foreign[col] = pd.to_numeric(df_foreign[col], errors='coerce').fillna(0).astype(int)
        net_buy = df_foreign["buy_qty"].sum() - df_foreign["sell_qty"].sum()
        return net_buy, df_foreign

    def get_pbar_tratio(self, code: str, retries: int = 10) -> Optional[dict]:
        """국내주식 매물대 비중(가격대별 체결량/비중) 조회 API. (한국어 주석: 신규 API 연동)"""
        params = {
            "FID_COND_MRKT_DIV_CODE": "J",
            "FID_INPUT_ISCD": code,
        ...
        return self.client.make_request(
            ...
            tr_id="FHKST01010400",  # 실제 트랜잭션ID는 필요시 수정
            params=params,
            retries=retries
        )
...
if __name__ == "__main__":
    ...
    try:
        ...
        test_and_log("get_stock_investor_detail", lambda: stock.get_stock_investor_detail(test_code))
        # estimate_accumulated_volume_by_top_members needs member_df
        try:
            member_df = stock.get_stock_member(test_code)
            if member_df is not None and not member_df.empty:
                result = stock.estimate_accumulated_volume_by_top_members({"output": member_df.iloc[0].to_dict()})
                print_df(result, "estimate_accumulated_volume_by_top_members")
                results.append(("estimate_accumulated_volume_by_top_members", True, None))
            else:
                print("❌ No member data to analyze for estimate_accumulated_volume_by_top_members")
                results.append(("estimate_accumulated_volume_by_top_members", False, "No member data"))
        except Exception as e:
            logging.exception("estimate_accumulated_volume_by_top_members failed")
            results.append(("estimate_accumulated_volume_by_top_members", False, str(e)))
