...
from kis.account.api import AccountAPI
from kis.stock.api import StockAPI
...
class KIS_Agent:
    ...
    def __new__(cls, *args, **kwargs):
        if cls._instance is None:
            cls._instance = super(KIS_Agent, cls).__new__(cls)
            # 초기화 코드 추가
            cls._instance._initialize(*args, **kwargs)
        return cls._instance

    def _initialize(self, account_info: dict, svr: str = 'prod', verbose: bool = False):
        # Initialize the core client
        self.client = KISClient(svr=svr, verbose=verbose)
    ...
    def __getattr__(self, name: str):
        ...
        for api in (self.stock_api, self.account_api, self.program_api, self.strategy_api):
            if hasattr(api, name):
