...
class KISClient:
    ...
    def refresh_token(self, svr: str = 'prod') -> None:
        auth(svr=svr)
    ...
    def make_request(
        ...
        for attempt in range(retries):
            ...
            try:
                ...
                if attempt < retries - 1:
                    ...
                    return {
                        'rt_cd': 'REQUEST_EXCEPTION',
                        'msg1': str(last_exception),
                        'error_type': 'RequestExceptionAfterRetry',
                    }
        logging.error(
        ...
        return {
            'rt_cd': 'UNKNOWN_ERROR',
            'msg1': str(last_exception) if last_exception else '모든 재시도 실패 후 알 수 없는 오류',
            'error_type': 'UnknownAfterRetries',
        }
