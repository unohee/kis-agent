...
| 지표 | 공식 스펙 | 실측 안정값 | 최종 적용값 | 개선도 |
|------|-----------|-------------|-------------|---------|
| **초당 요청 (RPS)** | 20 | 15-18 | **18** | 90% 안정성 |
| **분당 요청 (RPM)** | 1000 | 800-900 | **900** | 90% 안정성 |
| **캐시 적중률** | - | 80-95% | - | API 호출 대폭 감소 |
| **최소 간격** | - | 50ms+ | **50ms** | 연속 호출 안정성 |

## 1. 테스트 개요

### 테스트 환경
- **테스트 일시**: 2025년 9월 21일
- **API 버전**: 한국투자증권 OpenAPI v1
- **네트워크 환경**: 실제 운영 환경 (로컬 → KIS API)
- **테스트 브랜치**: `feature/rate-limit-testing`
- **PyKIS 버전**: v0.1.22

### 테스트 목적
- 공식 스펙 대비 실제 API Rate Limit 안정 한계 측정
- 프로덕션급 Rate Limiter 설정값 도출
- 캐시 시스템의 성능 효과 정량화
- 적응형 백오프 메커니즘 검증
