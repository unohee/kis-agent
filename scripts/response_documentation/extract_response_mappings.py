...
def extract_api_info(file_path: Path) -> Optional[Dict]:
    ...
    try:
        ...
        mapping_match = re.search(
            r"COLUMN_MAPPING\s*=\s*\{([^}]+)\}", content, re.DOTALL
        )
        if mapping_match:
        ...
        has_multiple_outputs = (
            "result1, result2" in content
            or "output1" in content
            and "output2" in content
        )
...
def main():
    ...
    for category, apis in grouped.items():
        ...
        with category_file.open("w", encoding="utf-8") as f:
            ...
            f.write("Auto-generated from examples_llm chk_*.py files\n")
            f.write('"""\n\n')
