...
def generate_response_class(
    ...
    if has_multiple_outputs:
        for i in range(1, output_count + 1):
            output_type = (
                f"{class_name}Output{i}" if output_count > 1 else f"{class_name}Output"
            )
...
'''
    ...
    for pykis_method, examples_api in sorted(PYKIS_TO_EXAMPLES_MAPPING.items()):
        # Stock 관련 메서드만 처리
        if pykis_method.startswith("get_") and any(
            x in pykis_method
            for x in [
                "stock",
                "daily",
            ...
            class_name_parts = [
                word.capitalize()
                for word in pykis_method.replace("get_", "").split("_")
            ]
...
'''
    ...
    account_methods = {
        k: v
        for k, v in PYKIS_TO_EXAMPLES_MAPPING.items()
        if "account" in k
        or "balance" in k
        or "ccld" in k
        or "psbl" in k
    ...
    for pykis_method, examples_api in sorted(account_methods.items()):
        ...
        # 클래스명 생성
        class_name_parts = [
            word.capitalize() for word in pykis_method.replace("get_", "").split("_")
        ]
        class_name = "".join(class_name_parts)
...
def main():
    ...
    print(
        f"Loaded {mappings_data['total_apis']} APIs from {len(mappings_data['categories'])} categories"
    )
    ...
    print(
        f"Account responses file: {account_output} ({account_output.stat().st_size} bytes)"
    )
